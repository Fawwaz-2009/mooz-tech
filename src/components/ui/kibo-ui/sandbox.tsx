...
import type { CodeEditorProps, PreviewProps, SandpackLayoutProps, SandpackProviderProps } from '@codesandbox/sandpack-react';
...
export const SandboxLayout = ({ className, ...props }: SandpackLayoutProps) => (
  <SandpackLayout className={cn('!h-full !rounded-none !border-none !bg-transparent', className)} {...props} />
);
...
export const SandboxTabs = ({ className, defaultValue, value, onValueChange, ...props }: SandboxTabsProps) => {
  ...
  return (
    <SandboxTabsContext.Provider value={{ selectedTab, setSelectedTab }}>
      <div className={cn('group relative flex size-full flex-col overflow-hidden rounded-lg border text-sm', className)} {...props} data-selected={selectedTab}>
        {props.children}
      </div>
...
export const SandboxTabsList = ({ className, ...props }: SandboxTabsListProps) => (
  <div className={cn('inline-flex w-full shrink-0 items-center justify-start border-b bg-secondary p-2 text-muted-foreground', className)} role="tablist" {...props} />
);
  ...
  return (
    <div
      ...
      className={cn(
        ...
        selectedTab === value ? 'h-auto w-auto opacity-100' : 'pointer-events-none absolute h-0 w-0 opacity-0',
        className
      )}
      {...props}
    />
...
export const SandboxCodeEditor = ({ showTabs = false, ...props }: SandboxCodeEditorProps) => <SandpackCodeEditor showTabs={showTabs} {...props} />;

export type SandboxConsoleProps = ComponentProps<typeof SandpackConsole>;

export const SandboxConsole = ({ className, ...props }: SandboxConsoleProps) => <SandpackConsole className={cn('h-full', className)} {...props} />;

export type SandboxPreviewProps = PreviewProps & {
  className?: string;
};

export const SandboxPreview = ({ className, ...props }: SandboxPreviewProps) => <SandpackPreview className={cn('h-full', className)} {...(props as any)} />;

export type SandboxFileExplorerProps = ComponentProps<any>;
// typeof SandpackFileExplorer

export const SandboxFileExplorer = ({ autoHiddenFiles = true, className, ...props }: SandboxFileExplorerProps) => {
  const Component = SandpackFileExplorer as any;
  return <Component className={cn('h-full', className)} autoHiddenFiles={autoHiddenFiles} {...props} />;
};
